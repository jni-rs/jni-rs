...
use crate::signature::{MethodSignature, parse_method_sig, parse_parameter_with_index};
use crate::str::lit_cstr_mutf8;
use crate::types::sig_type_to_rust_type_core;
...
struct NativeMethodStructInput {
    ...
    /// true if `export = true` or `extern` qualifier with inline signature
    export: bool,
}
...
impl NativeMethodStructInput {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        while !input.is_empty() {
            ...
            if is_prop {
                ...
                if lookahead.peek(rust_type) {
                    ...
                    java_type_opt = Some(lit.value());
                } else if lookahead.peek(name) {
                    // Parse: name = "methodName"
                    input.parse::<name>()?;
                    input.parse::<Token![=]>()?;
                    let lit = input.parse::<syn::LitStr>()?;
                    method_name = Some(lit.value());
                    ...
                    // Parse: export = true/false
                    input.parse::<export>()?;
                    input.parse::<Token![=]>()?;
                    let lit = input.parse::<syn::LitBool>()?;
                    export_opt = lit.value();
                ...
                if method_name.is_none() {
                    ...
                    method_name = Some(snake_case_to_lower_camel_case(&method_name_from_path));
                }
        ...
        if export_opt && java_type_opt.is_none() {
            return Err(syn::Error::new(
                input.span(),
        ...
        // Validate that error_policy is not specified with raw = true
        if is_raw_fn && error_policy_ident.is_some() {
            return Err(syn::Error::new(
                input.span(),
...
pub fn native_method_impl(input: TokenStream) -> Result<TokenStream> {
    ...
    let name_cstr = lit_cstr_mutf8(method_name);
    ...
    // Generate different code based on whether it's a raw or wrapped function
    let (wrapper_fn_name, native_method_block) = if input.is_raw_fn {
        // For raw/unsafe functions: type-check but don't wrap
        // user_fn_params matches raw_params for raw functions
        let block = quote! {
            // Type-check the raw function pointer by creating a type alias
            type _NativeMethodFn = for<#lifetime> extern "system" fn(
                #(#raw_params),*
            ) -> #return_type;
            let _type_check: _NativeMethodFn = #fn_path;

            // Safety: The function pointer is type-checked at compile time to match
            // the signature specified in the macro. The user has declared this as
            // `unsafe fn`, indicating they have written a raw extern "system" function.
            unsafe {
    ...
    if input.export {
        ...
        let mangled_name = create_jni_fn_name(java_class, method_name, Some(&jni_signature));
        ...
        Ok(quote! {
            {
                #export_name_attr
                pub extern "system" fn _export<#lifetime>(
                    #(#raw_params),*
                ) -> #return_type {
                    #wrapper_fn_name(#(#export_call_args),*)
                }
