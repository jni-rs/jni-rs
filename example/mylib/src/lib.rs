...
pub extern "system" fn Java_HelloWorld_hello<'caller_frame>(
    ...
    //
    // Before we can use this, we need `jni-rs` to know about the thread
    // attachment and have it map the pointer it into a non-transparent
    // `jni::JNIEnv` reference.
    unowned_env: JNIEnvUnowned<'caller_frame>,
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            ...
            let _ = thread::spawn(move || {
                ...
                jvm.attach_current_thread(JNIVersion::V1_4, |env| -> jni::errors::Result<()> {
                    for i in 0..11 {
