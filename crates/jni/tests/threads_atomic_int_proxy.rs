...
use jni::{
    AttachGuard, DEFAULT_LOCAL_FRAME_CAPACITY, JavaVM, errors, jni_str, objects::IntoAuto as _,
    strings::JNIStr, sys::jint,
};

use rusty_fork::rusty_fork_test;

mod util;
use util::{AtomicIntegerProxy, jvm};
...
fn test_destroy() {
    ...
    unsafe fn attach_current_thread_as_daemon<'local>(vm: &jni::JavaVM) -> jni::errors::Result<AttachGuard<'local>> {
        ...
        let res = unsafe {
            ((*(*jvm)).v1_4.AttachCurrentThreadAsDaemon)(
                jvm,
                &mut env_ptr,
                std::ptr::null_mut(),
            )
        ...
        Ok(unsafe { AttachGuard::from_unowned(env_ptr as *mut jni::sys::JNIEnv) })
    }
