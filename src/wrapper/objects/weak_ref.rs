    ...
    T: Into<JObject<'static>> + AsRef<JObject<'static>> + Default + JObjectRef + Send + Sync,
{
    ...
    ///
    /// Except for null reference; this API can only be used after a global [`JavaVM`] has been
    /// initialized (either via [`JavaVM::new`] or [`JavaVM::from_raw`]), such that
    /// [`JavaVM::singleton()`] will return `Some(JavaVM)`. Any time that you know that the current
    /// thread is attached (e.g. where you have an [`AttachGuard`] or [`JNIEnv`] reference) implies
    /// a [`JavaVM`] has been initialized.
    ...
    T: Into<JObject<'static>> + AsRef<JObject<'static>> + Default + JObjectRef + Send + Sync,
{
    fn drop(&mut self) {
        ...
        if !obj.is_null() {
            ...
            // been initialized to create a (non-null) WeakRef (WeakRef::new takes a JNIEnv
            // reference).
