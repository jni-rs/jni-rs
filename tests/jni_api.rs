...
pub fn with_local_frame() {
    attach_current_thread(|env| {
        let s = env
            ...
            .unwrap();
        let s = env.downcast_local::<JString>(s).unwrap();
...
pub fn call_new_object_unchecked_ok() {
    attach_current_thread(|env| {
        ...
        let jstr = env.downcast_local::<JString>(val).unwrap();
...
pub fn get_direct_buffer_capacity_wrong_arg() {
    attach_current_thread(|env| {
        let wrong_obj =
            unsafe { JByteBuffer::from_raw(env.new_string("wrong").unwrap().into_raw()) };
...
pub fn test_conversion() {
    attach_current_thread(|env| {
        ...
        let string = env.downcast_local::<JString>(obj).unwrap();
...
fn assert_exception_message(env: &mut JNIEnv, exception: &JThrowable, expected_message: &str) {
    let message = env
        ...
        .l()
        .unwrap();
    let message = env.downcast_local::<JString>(message).unwrap();
